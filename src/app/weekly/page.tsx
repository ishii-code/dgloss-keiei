/**
 * 週次予実（A3）。木〜水を1週サイクル。データはサーバーで getDashboardData 経由。
 */
...
export default async function Page() {
  const { weekly: w } = await getDashboardData();

  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-4">
        <span className="rounded-lg border border-line bg-white px-3 py-1.5 text-sm font-semibold text-ink">{w.unit}</span>
        <div className="flex items-center gap-2">
          <span className="text-xs text-muted-foreground">対象週</span>
          <span className="rounded-lg border border-line bg-white px-3 py-1.5 text-sm font-semibold text-ink">{w.targetWeek}</span>
        </div>
...
function WeeklyCard({ label, metric, kind }: { label: string; metric: WeeklyMetric; kind: "revenue" | "cost" | "profit" }) {
    ...
    <div className="rounded-2xl border border-line bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between">
        ...
        <div className="text-right">
          ...
          <div className={`text-lg font-bold tabular-nums ${ratioCls}`}>{ratio === null ? "—" : pct(ratio, 1)}</div>
        </div>
      </div>
      <div className="mt-3 space-y-0.5 text-xs">
        <div className="flex justify-between">
          <span className="text-muted-foreground">計画 {amt(plan)}</span>
          <span className={`tabular-nums ${diff < 0 ? "text-bad" : "text-good"}`}>差異 {diff < 0 ? "" : "+"}{amt(diff)}</span>
        </div>
