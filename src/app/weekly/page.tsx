    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-muted">対象週</span>
          <span className="rounded-lg border border-line bg-white px-3 py-1.5 text-sm font-semibold text-ink">
            {w.targetWeek}
          </span>
        </div>
        <span className="rounded-full bg-warn/10 px-2 py-0.5 text-[11px] font-bold text-warn">{w.note}</span>
        <span className="text-xs text-muted">木〜水を1週サイクル。売上=週次実績／コスト・営利=計画・見込のみ</span>
      </div>

      <div className="flex items-center gap-2">
        <h2 className="text-sm font-bold text-ink">主要指標</h2>
        <span className="text-xs text-muted">{w.targetWeek}（{w.asOf}）</span>
      </div>
      ...
      <section className="space-y-4">
        <div>
          ...
          <p className="text-xs text-muted">週次 計画 / 実績 / 見込（1W〜5W）</p>
        </div>
  ...
  return (
    <div className="rounded-2xl border border-line bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between">
        <div>
          <div className="text-xs text-muted">{label}（見込み）</div>
      ...
      <div className="mt-3 space-y-0.5 text-xs">
        <div className="flex justify-between">
          <span className="text-muted">計画 {amt(plan)}</span>
