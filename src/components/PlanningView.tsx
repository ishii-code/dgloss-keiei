...
/** 事業計画表示（FYトグル）。データは props（PlanBook）で受け取る。 */
import { useMemo, useState } from "react";
import { jpy } from "@/lib/format";
import { Button } from "@/components/shadcn/button";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/shadcn/table";
import type { Fiscal, PlanBook } from "@/types";

const sum = (a: number[]) => a.reduce((x, y) => x + y, 0);

export function PlanningView({ plan }: { plan: PlanBook }) {
  const { months, fiscals, base, growth } = plan;
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-xs text-muted-foreground">
        ※ FY2026 を実数、FY2027〜2030 は成長倍率で算出。DB接続時は会計/請求の計画値を表示。
      </p>
    </div>
  );
}
...
function UnitRows({ unit, revenue, cost, profit }: { unit: string; revenue: number[]; cost: number[]; profit: number[] }) {
